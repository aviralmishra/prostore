...
export async function approvePaypalOrder(
    ...
    try {
        ...
        if (
            !captureResponse ||
            captureResponse.id !== (order.paymentResult as PaymentResult)?.id ||
...
export async function getOrderSummary() {
    ...
    const salesData: SalesType = salesDataRaw.map((entry) => ({
        month: entry.month,
...
export async function updateOrderToPaidForCOD(orderId: string) {
    try {
